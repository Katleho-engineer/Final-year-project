...
def screened_data2(num):
    data = pd.read_csv(
        f"{raw_data_folder}/{num}")
    data = data[["Strain", "Stress(MPa)", "Time(sec)", "Force(kN)", "Jaw(mm)"]]
    return data


processed_data_folder = "/Users/admin/Library/CloudStorage/OneDrive-UniversityofCapeTown/final year project/DID " \
                        "WORK/supervised/cleaned data"
...
def results():
    ...
    print(dir_list)
    X_train = screened_data('0089')[["Strain", "Stress(MPa)"]]
    X_test = y_point(screened_data('0089'), trimmed_data('0089'))

    for num in dir_list:
        print(num)
        data_s_2 = screened_data2(num)

        y_train = data_s_2[["Strain", "Stress(MPa)"]]
        ...
        algo = LinearRegression()
        algo.fit(X_train, X_test)

        predict = algo.predict(y_train)
        predict = clusters(predict)
        predict = objective_2(predict, data_s_2)
